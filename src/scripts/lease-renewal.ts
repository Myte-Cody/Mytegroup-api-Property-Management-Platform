  ...
  try {
    ...
    if (result.details.length > 0) {
      ...
      result.details.forEach((detail, index) => {
        const status = detail.action === 'renewed' ? '✅' :
        ...
        logger.log(`   End Date: ${detail.oldEndDate.toISOString().split('T')[0]} → ${detail.newEndDate.toISOString().split('T')[0]}`);
    ...
    if (result.errors.length > 0) {
      ...
      result.errors.forEach((error, index) => {
        logger.error(`${index + 1}. Lease ${error.leaseId.substring(0, 8)}... (Tenant: ${error.tenantId.substring(0, 8)}...)`);
